...
In this notebook, we will study a topic models estimated on the PDF of Newton's Principia (1687) which we just downloaded. Reading the book into R with `pdftools` and creating a document frequency matrix with `quanteda`:
...
Note that since we only look at one book in this simple example and that we treat each page as a document. Some of these pages will contain very little text. More advanced approaches could combine pages, analyse chapters as documents, or many different books.
...
It seems we have identified three broad themes: A topic that could be called "astronomy", one that contains many terms regarding geometry, equations, geometry and algebra which we could term "maths", and a last topic containing words related to forces we could term a discussion of "mechanics" (although of course large parts of the book are about this topic).
...
Note that if we had added covariates, we would have estimated a structural instead of a correlated topic model, beta would need some re-weighting as can be seen [here](https://github.com/bstewart/stm/blob/master/R/cloud.R). Yet, as many functions access beta for us, we can e.g. obtain the highest probability words for each topic more easily with e.g. `labelTopics(model, n = 20)$prob`.

What can we learn about the structure of the book with this "automated reading"? Let us smooth topic shares with a 10 page moving average and plot them:
...
This outcome could now e.g. be crosschecked by looking at the PDF which indeed e.g. indicates that the pages around 150 contain maths in large proportion.
...
Topics 1 and 3 are less negatively correlated, so it seems that mechanics terms such as "force" and astronomy terms such as "planets" appear together relatively more often. If pages were longer and number of topics larger, topics could be positively correlated.