...
Note that this has estimated an new model, so topics will be different:
...
For other topics, however, the results are less reasonable. While the purpose of this notebook is to illustrate functionalities of (structural) topic models with some real world data, it also emphasises how important careful model selection and evaluation are to obtain more robust results such as in projects like http://j.mp/lda-congress-demo. Some exemplary areas and questions that could be explored here would e.g. be:
