---
title: 'MY459: Lab Session 2'
author: "Ken Benoit, Pablo Barbera, Christian Mueller"
...
```{r, error = TRUE}
txt <- "From 10k packages, quanteda is a text analysis package, for analysing texts."
char_wordstem(txt)
...
We already saw it earlier how to use `glob` pattern matching to identify documents based on whether they mention specific strings (with additional wildcard characters). Here's a few more examples of how these work in combination with `kwic`:

```{r}
kwic(data_corpus_inaugural, 'immigrant?', valuetype="glob") ## will match immigrant, immigrants
kwic(data_corpus_inaugural, 'immigra*', valuetype="glob") ## will match immigrants, immigration, immigrants
```

A more powerful pattern machting tool is called "regular expression". You can learn more about regular expressions [here](http://www.zytrax.com/tech/web/regex.htm). Regular expressions let us develop complicated rules for both matching strings and extracting elements from them.

For example, we could look at speeches that mention one of different words using the operator "|" (equivalent to "OR")

```{r}
kwic(data_corpus_inaugural, 'immigration|immigrants', valuetype="regex")
```
...
head(kwic(data_corpus_inaugural, 'over', valuetype="regex"))
head(kwic(data_corpus_inaugural, '^over', valuetype="regex"))
tail(kwic(data_corpus_inaugural, 'over$', valuetype="regex"))
tail(kwic(data_corpus_inaugural, '^over$', valuetype="regex"))

# but we can also do this:
tail(kwic(data_corpus_inaugural, 'over', valuetype="fixed"))

# making it a bit more complex...
head(kwic(data_corpus_inaugural, 'secur', valuetype="regex"))
head(kwic(data_corpus_inaugural, '^secur.{1,2}$', valuetype="regex"))
```
