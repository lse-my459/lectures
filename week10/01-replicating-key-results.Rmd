---
title: "Replicating some key results with word embeddings"
...
The pre-trained GloVe embeddings can be downloaded from [here](https://nlp.stanford.edu/projects/glove/). This notebook uses the embeddings with 100 dimensions as they allow for faster computations. Yet, the higher dimensional embeddings on average yield better results. Note that some of the outcomes here will change when using embeddings of different dimensions.
...
similar <- function(word, top_n = 5, embedding_matrix = embeddings) {
  ...
  # Delete the word from the search query which has a similarities of 1
  most_similar_words <- most_similar_words[!(most_similar_words %in% word)]
...
analogies <- function(a, b, c, top_n = 1, embedding_matrix = embeddings) {
  ...
  # Obtain the most similar words
  most_similar_words <- names(similarities_to_all_words)[1:(top_n + 4)]
...
```{r}
does_not_fit(c("armstrong", "moon", "hesse"))
```

Who was not a writer?

```{r}
does_not_fit(c("literature", "armstrong", "hesse"))
```
